 ...
 * @param {string} status
 * @returns {Promise<LeapRequest|null>}
 */
const findLatestByMenteeAndStatus = (menteeId, status) => {
  return LeapRequest.findOne({ mentee: menteeId, status }).sort({ createdAt: -1 });
};

/**
 * Creates and persists a new leap request document.
 * @param {Object} data
 * @returns {Promise<LeapRequest>}
 */
const create = (data) => {
 ...
 * @param {Object} filter
 * @returns {Promise<Array<LeapRequest>>}
 */
