 ...
 * Receives the Mongoose model as an injected parameter. Contains no business constraints or validation gates.
 */

const INDUSTRY_STATS_LIMIT = 12;

const createMentorProfileRepository = (MentorProfile) => {
  /**
  ...
  const findSimilarPublishedMentors = (excludeUserId, skills, limit) => {
    return MentorProfile.find({
      ...
      .populate("user", "name email")
      .select(
        "user currentRole company skills profilePicture avgRating industry hourlyRate",
      )
      .limit(limit)
      .lean();
  };
...
module.exports = createMentorProfileRepository;
