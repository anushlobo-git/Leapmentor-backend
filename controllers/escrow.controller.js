 ...
 * @description Decoupled request and response interface mapping HTTP gateway triggers
 * straight into active escrow services via parameter dependency injection.
 */
...
const createEscrowController = ({ escrowService }) => {
   ...
   * @access  Private (Mentee Only)
   */
  const payAdditional = catchAsync(async (req, res, next) => {
   ...
   * @access  Private (Mentee Only)
   */
  const release = catchAsync(async (req, res, next) => {
   ...
   * @access  Private (User)
   */
  const getCommissionRate = catchAsync(async (req, res, next) => {
