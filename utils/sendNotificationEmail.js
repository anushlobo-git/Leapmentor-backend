...
// Grey rounded info card — label + bold value + optional extra HTML
...
// CTA button with optional sub-caption
...
// Numbered step list used in onboarding emails
...
// Mentor + mentee participant block
...
// Standard blue-bordered slot rows
...
// Single slot row with custom colour scheme (used in cancel / reschedule)
...
// Common blue gradient used by most emails
...
const sendConnectRequestEmail = async ({
  ...
  const html = wrapEmail(`
    ...
    <div class="email-body" style="padding:24px 32px;">
      ...
      ${buildCTA(dashboardLink, "View Request", `Opens your <strong>Requests</strong> tab directly`)}
    </div>
    ${FOOTER}
  `);
...
const sendRequestAcceptedEmail = async ({
  ...
  const html = wrapEmail(`
    ...
    <div class="email-body" style="padding:24px 32px;">
      ...
      ${buildCTA(dashboardLink, "Complete Payment", `Opens your <strong>Session History</strong> tab directly`)}
    </div>
    ${FOOTER}
  `);
      ...
      <div style="background:#f8fafc;border-radius:12px;padding:18px;margin-bottom:18px;border:1px solid #e2e8f0;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
          ...
          <tr>
            ...
            <td style="font-size:13px;font-weight:600;color:#1e293b;text-align:right;padding:5px 0;">&times; ${sessionCount}</td>
          </tr>
          <tr>
            <td style="font-size:13px;color:#64748b;padding:5px 0;">Platform fee (${commissionRate}%)</td>
            <td style="font-size:13px;font-weight:600;color:#f59e0b;text-align:right;padding:5px 0;">deducted by platform</td>
          </tr>
          <tr>
            <td colspan="2"><div style="border-top:1px solid #e2e8f0;margin:8px 0;"></div></td>
          </tr>
          <tr>
            <td style="font-size:14px;font-weight:700;color:#0f172a;padding:5px 0;">You will receive</td>
            <td style="font-size:14px;font-weight:700;color:#16a34a;text-align:right;padding:5px 0;">${mentorPayout} tokens</td>
          </tr>
        </table>
      </div>
    </div>
...
const sendSlotCancelledEmail = async ({
  ...
  const buildHtml = (recipientName) =>
    ...
    <div class="email-body" style="padding:24px 32px;">
      ...
      <div style="margin-bottom:18px;">
        ...
        ${buildColoredSlotRow(slot, "#dc2626", "#fef2f2", "#fecaca", "CANCELLED")}
      </div>
  ...
  if (mentorResult.status === "rejected")
    logger.error("Mentor slot cancelled email failed", {
      message: mentorResult.reason?.message,
      to: mentorEmail,
    });
  if (menteeResult.status === "rejected")
    logger.error("Mentee slot cancelled email failed", {
...
const sendSlotRescheduledEmail = async ({
      ...
      <div style="margin-bottom:18px;">
        ...
        <div class="slot-row" style="display:flex;align-items:center;justify-content:space-between;
          ...
          <span style="font-size:11px;font-weight:700;color:#dc2626;background:#fee2e2;
            padding:3px 8px;border-radius:6px;white-space:nowrap;display:inline-block;flex-shrink:0;align-self:center;">OLD</span>
        </div>
        ...
        <div class="slot-row" style="display:flex;align-items:center;justify-content:space-between;
          ...
          <span style="font-size:11px;font-weight:700;color:#16a34a;background:#dcfce7;
            padding:3px 8px;border-radius:6px;white-space:nowrap;display:inline-block;flex-shrink:0;align-self:center;">NEW</span>
        </div>
      </div>
  ...
  if (mentorResult.status === "rejected")
    logger.error("Mentor slot rescheduled email failed", {
      message: mentorResult.reason?.message,
      to: mentorEmail,
    });
  if (menteeResult.status === "rejected")
    logger.error("Mentee slot rescheduled email failed", {
      message: menteeResult.reason?.message,
      to: menteeEmail,
    });
};
...
const sendAdditionalSlotEmail = async ({
  ...
  const buildHtml = (recipientName) =>
    ...
    <div class="email-body" style="padding:24px 32px;">
      ...
      <div style="margin-bottom:18px;">
        ...
        ${buildColoredSlotRow(slot, "#16a34a", "#f0fdf4", "#bbf7d0", "NEW")}
      </div>
  ...
  if (mentorResult.status === "rejected")
    logger.error("Mentor additional slot email failed", {
      message: mentorResult.reason?.message,
      to: mentorEmail,
    });
  if (menteeResult.status === "rejected")
    logger.error("Mentee additional slot email failed", {
      message: menteeResult.reason?.message,
      to: menteeEmail,
    });
};
  ...
  const html = wrapEmail(`
    ...
    <div class="email-body" style="padding:24px 32px;">
      ...
      <div style="background:#f8fafc;border-radius:12px;padding:16px 18px;margin-bottom:18px;border:1px solid #e2e8f0;">
        ...
        ${buildStepList([
          "Admin reviews your resume and work documents",
          "Your profile is verified and activated",
          "You're ready to start mentoring on LeapMentor",
        ])}
      </div>

      ${buildCTA(dashboardLink, "View Dashboard", "Check your application status anytime")}
    </div>
    ${FOOTER}
  `);
    ...
    <div class="email-body" style="padding:24px 32px;">
      ${buildInfoCard(
        ...
        <span style="display:inline-flex;align-items:center;gap:10px;">
          <span style="font-size:15px;font-weight:700;color:#1e293b;">${mentorName}</span>
          <span style="font-size:11px;font-weight:700;color:#16a34a;background:#dcfce7;padding:3px 8px;border-radius:6px;">VERIFIED</span>
        </span>
      `,
      )}
      ...
      <div style="background:#f8fafc;border-radius:12px;padding:16px 18px;margin-bottom:18px;border:1px solid #e2e8f0;">
        ...
        ${buildStepList(
          [
            "Complete your mentor profile with bio, skills and expertise",
            "Set your availability so mentees can book sessions",
            "Start accepting connect requests from mentees",
          ],
          "#16a34a",
        )}
      </div>

      ${buildCTA(dashboardLink, "Complete Your Profile", "You're one step away from your first session")}
    </div>
    ${FOOTER}
  `);
...
const sendReportResolvedEmail = async ({
  ...
  const headerGradient = isResolved ? GREEN_GRADIENT : RED_GRADIENT;
  ...
  const html = wrapEmail(`
    ${buildHeader(headerGradient, `Report ${statusLabel}`, "An update on your report has been made by our team")}
    <div class="email-body" style="padding:24px 32px;">
      <div style="background:#f8fafc;border-radius:12px;padding:18px;margin-bottom:18px;border:1px solid #e2e8f0;">
        <div style="font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:1px;margin-bottom:12px;">
          Report Summary
